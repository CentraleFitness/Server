	...
	public PostDelete(Router router) {
		router.route(HttpMethod.POST, Protocol.Path.POST_DELETE.path).handler(routingContext -> {
			...
			try {
				...
				if (post.getField(Post.Field.POSTERID).toString().equals(user.getId().toString()))
					Database.delete_entity(Collections.Posts, Post.Field.ID, post.getField(Post.Field.ID));
