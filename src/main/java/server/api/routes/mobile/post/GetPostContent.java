    ...
    public GetPostContent(Router router) {
        router.route(HttpMethod.POST, Protocol.Path.GET_POSTCONTENT.path).handler(routingContext -> {
            ...
            label:try {
                ...
                ObjectId pictureId = (ObjectId) Optional.ofNullable(post.getField(Post.Field.POSTERID)).map(posterId -> {
					try {
	                	User u;
						u = (User) Database.find_entity(Collections.Users, User.Field.ID, posterId);
	                	if (u != null) return u.getField(User.Field.PICTURE_ID);
	                	Fitness_Center_Manager fCM = (Fitness_Center_Manager) Database.find_entity(Collections.Fitness_Center_Managers, Fitness_Center_Manager.Field.ID, posterId);
	                	if (fCM != null) return fCM.getField(Fitness_Center_Manager.Field.PICTURE_ID);
	                	Fitness_Center fC = (Fitness_Center) Database.find_entity(Collections.Fitness_Centers, Fitness_Center.Field.ID, posterId);
	                	if (fC != null) return fC.getField(Fitness_Center.Field.PICTURE_ID);
                ...
                sending.put(Protocol.Field.POSTICON.key, Optional.ofNullable(picture).map(pic -> pic.getField(Picture.Field.PICTURE)).orElse(null));
