...
public class PostLike {
	public PostLike(Router router) {
		router.route(HttpMethod.POST, Protocol.Path.POST_LIKE.path).handler(routingContext -> {
			...
			try {
				...
				if (like.size() > 0)
					...
					likes.add((ObjectId) user.getField(User.Field.ID));
