...
public class GetCustomPrograms {
    public GetCustomPrograms(Router router) {
        router.route(HttpMethod.POST, Protocol.Path.GET_CUSTOM_PROGRAMS.path).handler(routingContext -> {
            ...
            try {
                ...
                if (!Objects.equals(manager.getField(Fitness_Center_Manager.Field.TOKEN), received.get(Protocol.Field.TOKEN.key))) {
                    ...
                    if (center == null) {
                        ...
                        LinkedList<Database.Entity> custom_programs = Database.find_entities(Database.Collections.CustomPrograms, CustomProgram.Field.FITNESS_CENTER_ID, manager.getField(Fitness_Center_Manager.Field.FITNESS_CENTER_ID));

                        sending.put(Protocol.Field.CUSTOM_PROGRAMS.key, custom_programs);
                    }
                }
            }catch (Exception e){
                sending = new ResponseObject(true);
