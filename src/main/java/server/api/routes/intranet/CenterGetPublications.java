...
public class CenterGetPublications {
    public CenterGetPublications(Router router) {
        router.route(HttpMethod.POST, ProtocolIntranet.Path.CENTER_GET_PUBLICATIONS.path).handler(routingContext -> {
            ...
            try {
                ...
                if (!Objects.equals(manager.getField(Fitness_Center_Manager.Field.TOKEN), received.get(ProtocolIntranet.Field.TOKEN.key))) {
                    ...
                    if (center == null) {
                        ...
                        sending.put(ProtocolIntranet.Field.PUBLICATIONS.key, publications);
                    }
                }
            }catch (Exception e){
                sending = new ResponseObject(true);
