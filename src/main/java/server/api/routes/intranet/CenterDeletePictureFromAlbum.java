...
public class CenterDeletePictureFromAlbum {
    public CenterDeletePictureFromAlbum(Router router) {
        router.route(HttpMethod.POST, ProtocolIntranet.Path.CENTER_DECREASE_ALBUM.path).handler(routingContext -> {
            ...
            try {
                ...
                if (!Objects.equals(manager.getField(Fitness_Center_Manager.Field.TOKEN), received.get(ProtocolIntranet.Field.TOKEN.key))) {
                ...
                } else if (received.get(ProtocolIntranet.Field.PICTURE_ID.key) == null) {
                    sending = new ResponseObject(true);
                    ...
                    if (center == null) {
                        ...
                        Fitness_Center.Picture_Describe cur;
                        for(Iterator<Fitness_Center.Picture_Describe> i = album.iterator(); i.hasNext();) {
                            cur = i.next();
                            if (cur.getField(Fitness_Center.Picture_Describe.Field.PICTURE_ID) == received.get(ProtocolIntranet.Field.PICTURE_ID.key)) {
                                i.remove();
                                break;
                            }
                        }
