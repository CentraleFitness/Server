...
public class CenterAddPublication {
    public CenterAddPublication(Router router) {
        router.route(HttpMethod.POST, ProtocolIntranet.Path.CENTER_ADD_PUBLICATION.path).handler(routingContext -> {
            ...
            try {
                ...
                } else if ((text = (String) received.get(ProtocolIntranet.Field.TEXT.key)) == null) {
                    sending = new ResponseObject(true);
